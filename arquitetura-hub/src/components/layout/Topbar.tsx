...
export function Topbar({ onMenuClick, title }: TopbarProps) {
  ...
  const sectionName = user?.role === 'admin' ? 'Admin' : 'Jornada de Marca'
    ...
    <header className="h-14 border-b border-gray-100 bg-white flex items-center justify-between px-6 flex-shrink-0">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-sm text-gray-400">{sectionName}</span>
          {pageName && pageName !== 'Início' && (
            <>
              <ChevronRight size={13} className="text-gray-300" />
              <span className="text-sm font-semibold text-gray-900">{pageName}</span>
            </>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2.5 pl-3 border-l border-gray-100">
          <div className="text-right hidden sm:block">
            <p className="text-xs font-semibold text-gray-900 leading-tight">{firstName}</p>
            <p className="text-[10px] text-gray-400">
              {user?.role === 'admin' ? 'Mentor' : 'Mentorado(a)'}
            </p>
          </div>
          <div className="w-8 h-8 rounded-xl bg-[#7B2FBE] flex items-center justify-center text-white text-xs font-black flex-shrink-0">
